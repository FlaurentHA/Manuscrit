...
The purpose of this work is to apply the methods presented herebefore to a certain type of physical object, Quantum Spin Liquids (QSLs).
This concept was first introduced by P.W Anderson in 1973, in an attempt to indentify the ground state of an antiferromagnetic Heisenberg type interaction in a triangular lattice of spin-1/2.
He first proposed the Resonating Valence Bond (RVB) model as a possible ground state of this system. 
In this model, the spins do not order magnetically at low temperature, as would be expected in more conventional systems, but fluctuate.
...
Several properties, namely fractionalised excitations and long range quantum entanglement, have been identified to occur in such system and can help refine the definition. 
Such concerns are out of the scope of this work but should serve as an explanation to the challenges that the study of QSL present.
Another obstacle to the characterisation of such object is the difficulty to obtain reliable experimental data.

The interest of this work lies in the Kagome lattice, it consists in corner sharing triangle where metalic ions occupy the vertices.
More precisely, this work focuses solely on Herbertsmithite which is has long been considered to be a perfect realisation of such a lattice with an antiferromagnetic behavior.
This material has long been studied from the theoretical side, but a renewed interest has emerged at the beginning of the 2000s with the synthesis of high-quality crystals.
...
In a first description, a fragment presenting only the first neighbor coupling was introduced with three copper ions as shown on Figure \ref{fig:FragmentDFT}(a).
...
\begin{figure}[!ht]
    ...
    \begin{subfigure}{.5\textwidth}
      ...
      \label{fig:Struc2+4}
    \end{subfigure}
    \caption[Close environnment of intersite $Cu^{2+}$ in both structure obtained via DFT with the B3LYP functional]{Close environment of intersite $Cu^{2+}$ in both structure obtained via DFT with the B3LYP functional. The bond lengths are shown in Angstrom}
