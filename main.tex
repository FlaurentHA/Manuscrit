...
The most famous system exhibiting anisotroppy is based on a Mn$_12$ complex which helped define Single Molecule Magnets (SMMs).
...
As the goal of this thesis is the characterisation of the anisotropy, which is a direct consequence of spin-orbit coupling, it is crucial to find a way to incluse some of the relativistic effects in our calculation.
Following a already established procedure, a two-step approach is chosen for this work.
...
The spin-orbit hamiltonian matrix is computed based on the Breit-Pauli operator and then diagonalised to obtain new eigenvectors which are mixture of the different spin-orbit free states.
The computation of the full Breit-Pauli being complicated, it is usually reduced to a one-particule operator:

\begin{equation}
    \hat{H}^{SO}=\sum_{i} \xi_i(\vb r_i) \hat{l}_i \cdot \hat{s}_i
\end{equation}
where $\xi_i$ is the effective spin-orbit constant.
...
At this stage the angular momentum L is essentially quenched making the spin momentum S a "not-too-bad" quantum number, allowing the application of the ZFS model Hamiltonian formalism with the extraction of a $D$ axial parameter and rhombic terme $E$.
...
This interaction stems from the excitation of an electron from a d-orbitals to a higher-energy orbital as depicted on Figure \ref{excitattionanisotropie} and its magnitude rely on the difference in energy between the orbitals.
