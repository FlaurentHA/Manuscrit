...
The variation of case (a) is linear with respects to the field $F$ as expected from Stark effect which tend to stabilize the ground state when the applied field and dipolar moment share the same orientation.
...
Figure \ref{fig:Ni_D_Z} shows the evolution of the $D$ parameter as a function of the applied field $F$.
...
\begin{table}
    ...
    \begin{tabular}{l c c}
        ...
        \hline
         & $\qquad$&$D_{AB}$\\
         \hline
         Contribution of $T_3^+$ and $T_3^-$& $\qquad$& -1176.74X \\
...
\begin{table}[!h]
    ...
    $\bra{S_1}$   & 0 & 0 & 0 & 0 & 0 & 1075i & 0 & 556 & 0 & 1 \\
    $\bra{S_3} $  & 0 & 0 & 0 & 0 &1078i & 0 &  1077i & 0 & 3369 & 0 \\
...
Note that now, the $h_i$ terms are no longer zero which allows DMI via the coupling from eq \ref{eq:chemindmi}.
The same procedure of diagonalization as in section \ref{sec:NoField} is applied with the selection of certains state that may contribution to either the $D_{AB}$ or the DMI.
This allows for the determination of the impact of the electric field on the axial parameter and its behavior in presence of DMI.
...
%%%%%%%%%%%%%%%%%%This can lead to issues in convergence toward  
...
In a similar approach, a fragment was built to include the inter ion represented on figure \ref{fig:FragAv}.
It contains two layers of kagome planes with three $Cu^{2+}$ ions and their neighboring ions each while the intersite $Zn^{2+}$ is replaced by a copper ion.
In the structuraly perfect kagome lattice, the intersite is occupied by a $Zn^{2+}$ ion which is not prone to a Jahn-Teller effect with doubly occupied d-orbitals and allows for a $C_3$ symmetry axis.
Replacing this ion with a $Cu^{2+}$, which is prone to Jahn-Teller effect with a singly occupied d-orbital, induces structural deformation of the environment of the defect.
...
To account for the deformations, the fragment's structure was optimised via DFT calculations using the ORCA package with the PBE or B3LYP functionals.
...
In order to maintain the rigidity of the crystal structure only the coordination sphere, \textit{i.e} the hydroxide groups close to the defect, was left free during optimisation, while the positions of the remaining ions were constrained.
Depending on the functional used, two different types of structure were obtained.
The PBE functional calculations converges toward a structure with four short Cu-O bonds and two long ones, this structure will be referred as ``4+2'' structure.
While the B3LYP functional calculations converges toward the opposite with two short Cu-O bonds and four long ones and will be labelled ``2+4''.
To account for which stucture is the most stable, one would compare the final single point energy provided by the program at the end of the $SCF$ cycle but comparing energies obtained using different functional is not appropriate.
As such, a second set of optimisations was performed starting from the previously obtained PBE (B3LYP) structure but now using the B3LYP (PBE) functional.
Both calculations converge toward the ``4+2'' structure which is slightly lower in energy than the ``2+4'' structure with a difference of $\Delta E=100 K$ using the B3LYP functional.
As the two structures are quite close in energy, a difference lower than the estimated first neighbor coupling, both structure were explored for the extraction of $J$ couplings.

\begin{figure}[!h]
    \centering
    \begin{subfigure}{.5\textwidth}
      \centering
      \includegraphics[width=\linewidth]{Images/Struc4+2.png}
      \caption{``4+2'' structure}
      \label{fig:Struc4+2}
    \end{subfigure}%
    \begin{subfigure}{.5\textwidth}
      \centering
      \includegraphics[width=\linewidth]{Images/Struc2+4.png}
...
These distortions, together with the presence of a copper ion at the intersite, necessitate the definition of new $J_i$ couplings.
Table \ref{tab:DefautDist} shows the distance between the oxygens and the different $Cu^{2+}$ ions.
In both optimised structure, one type of oxygens move away from the in plane copper while the other move closer.
\begin{table}[!h]
    \centering
    \begin{tabular}{c | c c c}
        & X-ray & ``4+2'' & ``2+4''\\
...
This displacement gives rise to a second in plane coupling $J_1'$ that differs from the $J_1$ by the distance between the copper ions and bridging oxygen.
Focusing on a triad of $Cu{2+}$ from the same plane, two bridging oxygens share an equivalent position while the third one is different.
The $J_1$ coupling is defined between two $Cu{2+}$ ions bridged by one of the two equivalent oxygen for a total of two $J_1$ couplings in the triad while $J_1'$ denotes the last coupling possible.
The out of plane coupling is a bit more complicated as the two copper ions involved are bridged by two oxygens.
The coupling between tw $Cu_{2+}$ bridged by two equivalent oxygens is called $J_5$ and appears once in the triad involving the defect.
The coupling going through two non equivalent bridges is in turn referred as $J_5'$, this coupling appears twice in the triad.
...
Table \ref{tab:DefautJ} shows the extracted values using BS-DFT on both the optimised and $C_3$ structure for comparison.
The $C_3$ structure indicates a strong $J_1$ antiferromagnetic coupling that compares perfectely with those extracted previously and a non negligible out of plane ferromagnetic coupling $J_5$. 
The extractions on the optimised structure show a significant impact of the deformations on the first neighbor coupling.
Firstly, coupling in the plane decreases as oxygen atoms move away from the plane, whereas it increases as they move closer to it. 
...
Note that these displacements lead to large differences between couplings of the same nature.
