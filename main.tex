...
\end{equation}
This target space $S_T$ is chosen to be the same as the model space $S_0$.
...
\begin{figure}[h!]
    ...
    \caption{Orbital diagram for $d^6$ configuration in ideal $D_{5h}$ geometry}
    \label{Fer_config}
\end{figure}
...
This loss of symmetry greatly reduces energy splitting of $M_S$ component and may induce rhombicity in the system. Nevertheless, the 
...
With this in mind, a series of five Iron(II) complexes were synthesized in a heptacoordinated environment with different apical ligands.
\begin{figure}[h!]
    \centering
...
This chapter is the follow up of a previous study on a Nickel II complex exhibiting large axial anisotropy.
...
\begin{figure}[h!]
    ...
    \caption{Representation of the complex [Ni(Me6trenCl)](ClO4), Nickel in green, Chlorine in pink, Nitrogen in blue, Carbon in black and Hydrogen in white}
    \label{NiMe6tren}
\end{figure}
...
This degeneracy gives rise to a two-fold degenerate ground state which are carried by the two configuration depicted on figure \ref{NiMe6tren_config} in the $C_3$ geometry. 
These two states would be strongly coupled via first order spin orbit coupling resulting in a large splitting of the $M_S$ component of the ground state with a large value for the D term as strong as the spin orbit coupling constant of the nickel ion ($\xi_{Ni}$=644cm$^{-1}$).
...
The concluding remark of this study was that this complex shows a large axial anisotropy with $D$\~-120cm$^{-1}$ and a significant rhombic term $E$=1.6cm$^{-1}$. 
...
fragment trimère, présentation de la méthodologie + extraction d'une première valeur de J1

Ouverture à des fragments plus grands, 9CU,12Cu,13Cu permettant l'introduction de couplage à plus longue distance ainsi que le calcul d'un plus grand nombre de solutions.
Plein de système d'équation et donc plein d'extraction possible.

 9cu à écarter car trop de variance dûe au fait que les interaction font toutes intervenir des cuivres sur les bords.

Pour 13 Cu, beaucoup de solutions prise en compte, grosse variance quand les solutions principalement ferromagnétique sont prise en compte.
Si on les élimine, extraction consistante

Resultats 2 derniers fragments.

\subsubsection*{Out-of-plane coupling}
Présence de défaut à hauteur de 15\%....
