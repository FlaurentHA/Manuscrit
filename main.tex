...
Magnetic anisotropy, which causes single molecule magnet (SMM) behaviour, was discovered in the Mn12(acac) molecule in 1993. (cite Sessoli) 
...
These rationalisations, which also use crystal field theory and/or orbital models, enable a dialogue to be established with experimentalists specialising in synthesis, which has often proved very useful and productive. They have been used on numerous occasions to guide syntheses in the design of new compounds with improved properties. 
...
To achieve this, a procedure was developped for extracting values of the Dzyaloshinskii-Moriya pseudovector interaction $\vb{d_{AB}}$ and the symmetric exchange anisotropy $D_{ij}$-tensor. 
...
It was then thought that applying an external electric field could have a similar effect by acting on the electronic could itself rather than the nuclei's position.
...
Dynamic correlation was introduced using the DDCI3 method, performed using the CASDI code which is interfaced with the MOLCAS chain.
...
This concept was first introduced by P.W Anderson in 1973, in an attempt to indentify the ground state in a triangular lattice of spin-1/2 with an antiferromagnetic Heisenberg type interaction.
...
temperature series expansions, exact diagonalizations, tensor network methods, mean-field approaches, variational studies...).
...
Although unable to reproduce the collective effects of the crystal, methodologies have been developed using cuantum chemistry theories to extract the local interactions and validate the model.
This work aims to do exactly that.
The combination of DFT calculations and embedded cluster theory enables numerous isotropic couplings in the model and the geometric displacement induced by the defects to be considered.
...
Cristallographic studies have identified two types of defect present at low temperatures. 
...
Both calculations converged towards the ``4+2'' structure which was found to be slightly lower in energy than the ``2+4'' structure with an energy difference of $\Delta E=100 K$ using the B3LYP functional.
...
Whereas the symmetric structure has only one in-plane coupling, $J_1$, the displacements of the oxygen necessitate the definition of new couplings:
\begin{itemize}
\item $J_1'$ the in-plane coupling is defined between two $Cu^{2+}$ ions bridged by one of the two equivalent oxygens. It appears twice in the triad involving the defect.
\item $J_1''$ is the in-plane coupling involves the remaining oxygen, appearing once.
