...
\lhead{\textsc{University of Toulouse}}
\cfoot{\thepage}
...
Magnetic anisotropy, which causes single molecule magnet (SMM) behaviour, was discovered in the Mn$_{12}$(acac) molecule in 1993.\cite{sessoli_high-spin_1993} 
Since the first studies conducted on similar compounds (cite Fertruc), this subject has evolved considerably and is now the focus of numerous studies in both the fundamental field of quantum mechanics (quantum tunnelling, quantum interference, etc.) and more applied fields (spintronics, quantum computing, high data storage, etc.).
...
The interactions appearing in equation 1 are numerous and are not observable.
...
However, it is not easy to manipulate magnetic systems, which often lack polarisation, using an electric field.
...
Chapter~\ref{TheoryMethod} concerns the study of first-order spin-orbit coupling (SOC) and its implication on the models. 
...
The second complex has already been identified to exhibit large anisotropy values thanks to a strong first-order SOC.
In a follow-up study, our interest lies in the evolution of its anisotropic parameters under the effect of an external electric field.
...
The associated spin Hamiltonian is written\cite{noauthor_molecular_1995}:
...
\noindent where $J$ is the isotropic coupling constant found in the HDvV Hamiltonian from eq.\ref{Hheis}.
...
While this method provides the non-dynamical correlation, it fails to capture the correlation with the inactive electron and their reponse to excitations, called dynamic-correlation which plays a crucial work in the physics of magnetic systems.


\subsection{Perturbation Theory}

Perturbative treatment can be applied to extract the dynamical correlation brought by the mono and diexcitations that is left out from simple CASSCF calculation. 
...
usually at second-order of perturbation with CASPT2~\cite{noauthor_second-order_nodate} or NEVPT2~\cite{angeli_introduction_2001,angeli_n-electron_2002}. These two methods mainly differ from the choise of the zeroth-order Hamiltonian, CASPT2 rely on a monoelectronic Hamiltonian built from a one electron Fock operator that falls back to the Moller-Plesset Hamiltonian in single reference case. 
...
\end{equation}
Kohn and Sham introduced new definition of this functional by replacing the interacting system with a fictious system of N non interacting electrons\cite{kohn_self-consistent_1965}.% that reproduces the same ground state electron density $\rho(\vb r)$.
...
In parallel, a first-order SOC model was adopted, previously developed for low-symmetry iron~\cite{atanasov_detailed_2011} and nickel complexes.
...
This coupling stabilises the $m_s$=$\pm$1 of the $T^0$ ground state without affecting the $m_s$=0 component, which gives a negative contribution to the D term.
...
The QZVPP extended basis sets have been used for Ni atoms (14s10p5d4f2g), TZVP for other atoms (6s3p2d1f for N, 8s4p2d1f for Cl and 6s3p2d1f for C) and SVP for H (2s1p). 
...
Table~\ref{tab:Slope_D_Z} reports the slopes $dD/dF$ and $dE/dF$ for all three cases of calculations and their individual sums.
...
\begin{table}[h]
    ...
    \begin{tabular}{c | c | c c | c }
        ...
        \hline
    \end{tabular}
    \caption{Weights w($d_{xz}$) and w($d_{yz})$ of the determinants with single occupation in the respective orbitals, their difference $\Delta w(X-Y)$ and their overall variation $\Delta (\Delta w(X-Y))$ in all three case of calculation for $\vec{\vb{F}}\parallel Z$. }
    \label{tab:PoidsZ}
\end{table}
...
The application of the electric field along the Y direction tends to elongate the Ni-N$_3$ bond and concomitantly reduces the other Ni-N bond lengths in the XY plane.
...
\begin{itemize}
    ...
    In case (a) for $\vec{\vb{F}}\parallel Z$, the large increase in magnitude $|D|$ can be related to a large variation of the SOC while the energy difference $\Delta E$ between $T_0$ and $T_1$ decreases.
...
\par The spin Hamiltonian used to describe anisotropic behaviour in binuclear systems is as follow:
...
The continious electric field was added in the SEWARD stage of the calculation in MOLCAS,it was applied along the X,Y and Z directions, but most of the results reported here relates to the Y direction.
...
To assess the impact of dynamical correlation on the $D_{AB}$ parameter, calculations were performed with gradual inclusion of the DDCI3 corrections (energy and wave-function).
...
The following work if part of an effort to open up quantum chemistry methods to more theoretically oriented physical systems, in particular to the frustrated magnetism field.
The purpose of this work is to apply the methods presented herebefore to a physical object that has attracted the attention of theorician for a long time, Quantum Spin Liquids (QSLs).
This concept was first introduced by P.W Anderson in 1973\cite{anderson_resonating_1973}, in an attempt to indentify the ground state in a triangular lattice of spin-1/2 with an antiferromagnetic Heisenberg type interaction.
...
Consequently, unlike in more conventional antiferromagnetic systems, the spins do not order magnetically at low temperatures.
This absence of magnetic order would not come from thermal fluctuations but from purely quantum ones. 
Contraty to paramagnets, the disorder would be a direct consequence of higly correlated quantum states.
The pictures of magnetic disorder is a good enough starting point to define a QSL, but it remains imperfect.
Several properties have been identified as occurring in such systems, namely fractionalised excitations and long-range quantum entanglement. 
These properties can help to refine the definition. 
...
In a first approximation, it can described by one of the simplest Hamiltonians: the Heisenberg model, which consists in magnetic exchange couplings of S = 1/2 spins on neighboring sites of the Kagomé lattice.
...
These methods describe of the collective effects and aim to identify the characteristics of the ground state.
...
While magnetic susceptibility measurements show no evidence of magnetic ordering at low temperatures, some deviations from the behaviour of an ideal S=1/2 Heisenberg antiferromagnet have been observed.
...
Long-range couplings (beyond than nearest-neighbours) have been mentioned as a possible source, but more importantly the presence of defects in the structure with magnetic impurities has been considered.
...
Both of these avenues present a real challenge to theorican, but we believe that chemists may be able to provide answers.
Although unable to reproduce the collective effects of the crystal, methodologies have been developed using quantum chemistry theories to extract the local interactions and validate the model.
This work aims to do exactly that.
The combinated use of embedded cluster method and DFT or wave-function methods allows the estimation of multiple isotropic coupling.
The deformation of the structure following the consideration of defects will be studied as well using DFT for geometry optimisation.
And finally, anisotropic exchange interaction will be extracted using the procedure detailed in the previous chapter.


\subsection*{Crystal Structure}

Herbertsmithite has the formula $ZnCu_3(OH)Cl_2$, its crystal stucture is trigonal rhombohedral (space group $R{\overline{3}m}$) is represented in Figure~\ref{Herbert}.
The Kagomé lattice sites are occupied by Cu$^{2+}$ ions which forms $CuO_4$ plaquettes with the bridging oxygens. 
Each Kagomé planes are separated by Cl$^-$ ions and Zn$^{2+}$ ions forming distorted ZnO$_6$ octahedra. 
Note that the hidroxide groups, and Zn$^{2+}$ ions, alternate above and below the Kagomé plane.
...
The source of this deviation is believed to come from the environment of ions taking part in the interaction. 
The extraction of these couplings requires to flip the spin of at least one of the ion at the edge of the fragment whose neighboring atoms are not always explicitely computed (treated with ECP's).
This has a significant impact on the convergence which leads to large variation relative to the small values obtained.
It was decided to increase the size of the fragment to twelve and thirteen copper ions (fragment (c) and (b) from Figure~\ref{fig:FragmentDFT}) so that there would be ways account for these interactions between magnetic centers far from the edges and thus weakly affected by the substitution of atoms by ECPs.
...
This confirm the the use of models composed of only one $J$ interaction between nearest-neighbors, the neglect of other couplings should cause no more than a small deviation.

\subsection{Out-of-plane coupling}

Another preoccupation of the model is the occupation disorder. 
Cristallographic studies have identified two types of defect present at low temperatures in the crystal samples. 
...
As such, our study focus on the first type of disorder.
...
This cluster contains two layers of Kagome planes, each with three Cu$^{2+}$ ions and hydroxo ions of their coordination sphere while the interstitial Zn$^{2+}$ ion is replaced by a copper ion.
...
In order to determine the most stable structure, a second set of optimisations was performed starting from the previously obtained PBE (B3LYP) structure but now using the B3LYP (PBE) functional.
...
The $C_3$ structure indicates a strong $J_1=184.22K$ antiferromagnetic coupling which is consistent with previously extracted values, as well as a non negligible out-of-plane ferromagnetic coupling $J_5=-80.64K$ (the symmetric structure has only one type of out of plane coupling). 
...
%\end{landscape}
%\begin{landscape}
    \includepdf{Images/matrice_fullpage.pdf}
%\end{landscape}
...
The slope $dD/dF$ is nearly 100 times greater than that calculated and measured for a Mn(II) complex, the study of which was very recently published (cite Nature Chemistry).
...
The important conclusion from this work is that all the parameters studied, i.e. the symmetric tensor of single-nuclear complexes, the symmetric exchange tensor and the Dzyaloshinskii-Moriya tensor that appear in poly-nuclear complexes, contribute to magnetoelectric coupling.
...
It should be noted that, unlike several strongly correlated materials studied previously (cuprates (Carmen and Coen), nickelates (Bastardis), manganites (Bastardis), vanadates (Nico), etc.), this material has proven particularly difficult to describe.
...
Here too, several interesting physicochemical results were obtained:
\begin{itemize}
    \item[-]The coupling with interplanar Cu2+ impurities resulting from an exchange between interplanar $Zn^{2+}$ and intraplanar $Cu^{2+}$ is ferromagnetic and of the same order of magnitude as the nearest-neighbour $J$. 
    ...
    \item[-]The presence of these impurities, which occupy a C3 symmetry site and produce $Cu^{2+}$ prone to Jahn-Teller effect, induces local distortion of the lattice.
    These distortions cause two very different couplings between the first-neighbour intra-plane $Cu^{2+}$ ions, with values of 92 $K$ and 264 $K$ respectively (values to be verified).
    ...
    It should be noted that this source of anisotropy is often neglected in studies of collective properties and when taken into account, it is generally reduced to its single component along the z-axis, i.e. perpendicular to the Kagomé lattice plane.
...
Furthermore, the study of the impact of the electric field in such a system could reveal possible interference effects between all the sources of anisotropy in the Hamiltonian of equation 1. 
Finally, an important perspective concerns strongly correlated materials.
Other materials related to Herbertsmithite, such as paratacamite (ref) or claringbullite (ref), are worth studying, on the one hand to test commonly used models and possibly propose alternative models, and on the other hand for the challenge posed by studying these systems, which are so difficult to describe using our best methods.
