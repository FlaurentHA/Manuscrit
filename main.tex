...
\begin{equation}\label{Hheis}
    \hat{H}=\sum_{i,j} J_{ij} \hat{\vb S_i} \cdot \hat{\vb S_j}
\end{equation} 
    where $J_{i,j}$ is the coupling constant, $\hat{\vb S_i}$ and $\hat{\vb S_j}$ are the spin operators working on site $i$ and $j$.
    Several conventions exist for this Hamiltonian, with negative sign and/or with a factor of 2 in front. As the intention of this work %
    was to communicate with theoretical physicist we will use remove any factor (\ref{Hheis}) unless mentioned. The coupling constant $J$ %
     can be either positive or negative depending on the magnetic properties of the system, a negative value indicate ferromagnetism with %
     magnetic moment aligned while positive indicate antiferromagnetism with opposite alignement of magnetic moments.
    This Hamiltonian is regarded as a Spin Hamiltonian as it only invoke the spin degree of freedom of the system and is still %
    to this day used in the description of the isotropic coupling in magnetic systems.
...
\subsection{Single centre Anisotropy}
    ...
    More specifically on mononuclear systems (only one magnetic centre) with a ground state of spin larger or equal to one, %
    a lift of degeneracy between the different $M_S$ component of a same $S$ state can be observed even in the %
    absence of magnetic field, this phenomenon is called Zero-Field-Splitting.
...
For a ground state with spin larger than one and a half, other terms may appear but will not be reported in these work as the case never occured.

In the case of a triplet S=1 ground statesuch as a nickel Ni(II) complex that will be studied, the matrix representation of hamiltonian \ref{SDS} in a random orientation:
...
Opening the study to multiple magnetic center introduces weak couplings between the magnetic moments of each center. This coupling between two sites A and B, of spin 1/2 both,is called exchange interaction and is described by the following spin Hamiltonian.

\begin{equation}
    \hat{H}_{AB}=\hat{\vb S}_A\cdot \overline{\overline{A}} \cdot \hat{\vb S}_B
\end{equation}
The spin-spin interaction tensor $\overline{\overline{A}}$ can be decomposed in three component.
\begin{equation}
...
So far, we have assumed the colinearity of spin moments, the introduction of this last term now favor a \textit{spin canting} where the spin moment are no longer parallel or antiparallel resulting in weak ferromagnetism in otherwise antiferromagnetic systems.


With magnetic centers carrying more than one magnetic electron, local tensors need to be added.
...
\begin{equation}
    ...
    +\sum_{A=1}^{M}\sum_{B>A}^{M}\frac{Z_AZ_B}{|R_A-R_B|}
\end{equation}
...
We introduce the shorthand notation for such determinant $\Psi(x_1,x_2,...,x_N)=\ket{\chi_1\chi_2...\chi_k}$ showing only the diagonal elements of the determinant.
...
%ATTENTION DEMANDER%
As useful as this method is it fails to describe corretly some properties like the dissociation energy in $H_2$ showing sign of 
